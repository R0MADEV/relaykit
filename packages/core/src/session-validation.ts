...
export function validateLoginCredentials(credentials: LoginCredentials): void {
  const hasEmptyValue = [credentials.homeserver, credentials.username, credentials.password].some(
    value => !value.trim()
  );
  if (hasEmptyValue) {
...
export function validateSession(session: Session): void {
  const hasEmptyValue = [session.homeserver, session.userId, session.accessToken].some(
    value => !value.trim()
  );
  if (hasEmptyValue) {
