...
async function main() {
  ...
  if (everythingKnown.length > 60) {
    throw new Error(
      `The window was asked for forty and the client knows ${everythingKnown.length} conversations`
    );
  }
  ...
  if (first.some(item => item.id === undefined))
    throw new Error("A conversation with no identifier came back");
  ...
  if (!pollsThere.some(poll => poll.id === started.id)) {
    throw new Error(
      `A poll started inside the window is not among the ${pollsThere.length} the conversation has`
    );
  }
  ...
  const farEnoughDown = everything.filter(
    item => item.lastMessage !== undefined && item.membership === "join" && !held.has(item.id)
  );
  ...
  const readUpTo = (await untouched.conversations.list({ limit: 200 })).find(
    item => item.id === farDown.id
  )?.lastReadMessageId;
  ...
  if (forSettings.some(item => item.id === neverTouched.id)) {
    throw new Error(
      `The one being changed is inside the window of ${forSettings.length}, so this proves nothing`
    );
  }
  ...
  console.log(
    `RelayKit window smoke check passed (caught up in ${catchingUp.took} ms, and reached past the window)`
  );
}
