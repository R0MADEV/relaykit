...
class FullStorage extends InMemoryStorage {
  async saveMessages() {
    throw new Error("QuotaExceededError: no queda sitio");
  }
...
test("running out of room is reported, not swallowed in silence", async () => {
  ...
  assert.ok(
    avisos.some(aviso => aviso.includes("QuotaExceededError")),
    `nadie aviso: ${avisos}`
  );
  await client.stop();
});
...
test("a store that cannot read either sends you to the homeserver instead of breaking", async () => {
  class UnreadableStorage extends FullStorage {
    async getConversations() {
      throw new Error("QuotaExceededError: no se puede leer");
    }
    async getMessages() {
      throw new Error("QuotaExceededError: no se puede leer");
    }
  ...
  assert.deepEqual(
    (await client.messages.list(conversation.id)).map(m => m.body),
    ["lo dicho"]
  );
